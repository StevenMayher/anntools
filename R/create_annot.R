...
create_annot = function(mat = NULL,
                        mat_file = NULL,
                        mat_dir = getwd(),
                        count_import_opts = NULL,   # If you have additional arguments you'd like to pass to importing of a matrix, provide the options here as a list(). See sc_import() for details.
                        vars_or_obs = NULL,         # May be either "vars" or "obs". Necessary if using a matrix file / object - select "vars" (i.e. variables) if you would like to create an annotation dataset for the column variables, and "obs" (i.e. observations) if you wish to create one for the row observations.
                        annot_obj = NULL,
                        annot_file = NULL,
                        annot_dir = getwd(),
                        annot_import_opts = NULL,   # If you have additional arguments you'd like to pass to importing of a matrix, provide the options here as a list(). See sc_import() for file details
                        annot_join = NULL,
  ...
  if(!is.null(mat_file)){
    matrix_import = sc_import_count(file = mat_file,
                                    dir = mat_dir,
  ...
  if(!is.null(annot_file)){
    annot_import = sc_import_annot(file = annot_file,
                                   dir = annot_dir,
                                   annot_import_opts)
  }else{
    annot_import = NULL
  ...
  if(!is.null(annot_obj)){
    annot_object = as.list(annot_obj)
  }else{
    annot_object = NULL
  }
      ...
      if(is.list(pseudotimes)){
        ...
        if(length(pseudotimes) == 1){
          if(select_annot_cols[3]){
            if(pseudotimes %in% y){
              if(is.numeric(unlist(annot_base[pseudotime], use.names = FALSE))){
  ...
  if(!is.null(annot_base) && !is.null(matrix)){
    ...
    ### VERIFY THAT COUNT MATRIX AND ANNOTATION OBJECT HAVE MATCHING DIMENSIONS ###
    if(!is.null(vars_or_obs)){
      if(vars_or_obs == "vars"){
        if(dim(matrix)[[2]] != x){
          if(remove_missing){
            ...
            if(dim(matrix)[[2]] < x){
              ...
              if(!is.null(IDs) && !is.list(IDs) && is.vector(IDs) && length(IDs) == 1 && IDs %in% names(annot_base)){
                ...
                count_index = data.frame(dimnames(matrix)[[2]])
                names(count_index) = sprintf("%s", IDs)

                annot_base = data.frame(do.call(cbind, annot_base), check.names = FALSE)
                annot_base = dplyr::inner_join(annot_base, count_index, by = IDs)

                if(dim(annot_base)[1] == dim(matrix)[[2]]){
                  annot_base = base::as.list(annot_base)

                }else if(dim(annot_base)[1] == 0){
                  stop(sprintf("Count matrix variable names and Annotation field IDs in annotation column %s specified via 'IDs' do not contain any matches - check count matrix variable names and annotation object column %s and try again", IDs, IDs))
                }else{
                  stop(sprintf("Annotation object is missing count matrix variables. Check that correct count matrix and annotation object have been provided, and if so, check count matrix variables and annotation ID field %s for inconsistencies.", IDs))
                }
              }else{
                stop("Cannot remove missing annotation observations - please provide a valid ID column name from the provided annotation object via 'IDs' to use to compare against the provided count matrix's varable names.")
              }
            }else{
              stop(sprintf("Annotation object %s contains less observations than the provided count matrix - add missing observation(s) to provided annotation object or subset count matrix to match provided annotation data and try again."))
            }
          }else{
            if(dim(matrix)[[2]] > x){
              stop("Dimension mismatch - the number of variables in the provided count matrix is greater than the number of observations in the provided annotation object - either subset the count matrix to match provided annotation object or append missing observations to annotation object and try again.")
            }else{
              stop("Dimension mismatch - annotation object contains more observations than the number of variables in the provided count matrix - double-check that correct objects have been provided, and if necessary subet annotation object to match count matrix by setting 'remove_missing' to TRUE.")
            }
          }
        }
      }else if(vars_or_obs == "obs"){
          ...
          if(remove_missing){
            ...
            if(dim(matrix)[[1]] < x){
              ...
              if(!is.null(IDs) && !is.list(IDs) && is.vector(IDs) && length(IDs) == 1 && IDs %in% names(annot_base)){
                ...
                count_index = data.frame(dimnames(matrix)[[1]])
                names(count_index) = sprintf("%s", IDs)
                ...
                if(dim(annot_base)[1] == dim(matrix)[[1]]){
                  ...
                  stop(sprintf("Count matrix observation names and Annotation field IDs in annotation column %s specified via 'IDs' do not contain any matches - check count matrix observation names and annotation object column %s and try again", IDs, IDs))
                }else{
                  stop(sprintf("Annotation object is missing count matrix observations. Check that correct count matrix and annotation object have been provided, and if so, check count matrix observations and annotation ID field %s for inconsistencies.", IDs))
                }
              }else{
                stop("Cannot remove missing annotation observations - please provide a valid ID column name from the provided annotation object via 'IDs' to use to compare against the provided count matrix's observation names.")
              }
            }else{
              stop(sprintf("Annotation object %s contains less observations than the provided count matrix - add missing observation(s) to provided annotation object or subset count matrix to match provided annotation data and try again."))
            }
            ...
            if(dim(matrix)[[1]] > x){
              stop("Dimension mismatch - the number of observations in the provided count matrix is greater than the number of observations in the provided annotation object - either subset the count matrix to match provided annotation object or append missing observations to annotation object and try again.")
            }else{
              stop("Dimension mismatch - annotation object contains more observations than the number of observations in the provided count matrix - double-check that correct objects have been provided, and if necessary subet annotation object to match count matrix by setting 'remove_missing' to TRUE.")
            }
          }
        }
      }
    }else if(is.null(vars_or_obs) || vars_or_obs != "uns"){
      stop("Please specify a valid annotation structure type to generate via 'vars_or_obs' - see documentation for valid options.")
    }
    ...
    if(is.null(color_by)){
      if(!is.null(pseudotime_labs)){
        ...
        colors_key = ann_color_palette(color_palette = color_palette,
                                       color_by = unique(unlist(pseudotime_labs, use.names = FALSE)))

        color_list = list(colors_key[match(pseudotime_labs,table = names(colors_key))])
      ...
      if(!is.null(annot_base)){
        if(length(color_by) == 1){
          if(color_by %in% names(annot_base)){
            colors_key = ann_color_palette(color_palette = color_palette,
                                           color_by = unique(annot_base[[color_by]]))

            color_list = list(colors_key[match(color_by, table = names(colors_key))])
  ...
  if(return_as == "list"){
    ...
    annot_data = data.frame(do.call(cbind, annot_data))
    return(annot_data)
  }
